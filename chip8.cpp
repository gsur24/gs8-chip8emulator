...
void chip8::emulateCycle() {
  ...
  case 0xD000:
    { // DXYN: Draw a sprite (X, Y) size (8, N) at location I
      if (debug)
  ...
  case 0xF000: // multi-case opcode
    ...
    case 0x0033:
      { // Stores a binary coded decimal value VX in I
        // I+1, and I+2
        ...
        if (debug) {
          cerr << opcode << ": storing binary coded decimal value" << " in I, I+1, I+2\n";
        }
    ...
    case 0x0065:
      { // fills V0 to VX with values from I
        int x = (opcode & 0x0F00) >> 8;
        for (int i = 0; i < x; i++) {
          V[i] = memory[I + i];
        }
        ...
        if (debug) {
          cerr << opcode << ": filling V0 to VX with values from " << "I\n";
        }
